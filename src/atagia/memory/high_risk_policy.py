...
HIGH_RISK_CHAT_POLICY_INSTRUCTION = (
    ...
    "ceiling, status, and explicit disclosure conditions. When the current "
    "authenticated user asks for a permitted logistics code and retrieved "
    "context supplies a current value, answer the current value directly; do "
    "not route to a secure reveal flow unless the target is a "
    "`pin_or_password` secret literal or an explicit disclosure condition blocks "
    "ordinary chat. When withholding a raw secret literal, say that you cannot "
    "disclose that secret in chat and direct the user to a host-managed secure "
    "reveal or verification flow. You "
