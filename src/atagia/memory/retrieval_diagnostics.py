...
def build_retrieval_sufficiency_diagnostic(
    *,
    ...
    verbatim_evidence_search_count = _count_channel_candidates(raw_candidates, "verbatim_evidence_search")
    ...
    if not raw_candidates:
        ...
        verbatim_evidence_search_count=verbatim_evidence_search_count,
    ):
        state = "insufficient_need_more_raw_evidence"
        confidence = 0.82
        rationale_codes.append("raw_evidence_requested_no_direct_candidates")
        would_expand_channels.append("verbatim_evidence_search")
        would_abstain = True
    ...
    return RetrievalSufficiencyDiagnostic(
        ...
        verbatim_evidence_search_candidate_count=verbatim_evidence_search_count,
...
def _has_channel(candidate: dict[str, Any], channel: str) -> bool:
    if channel == "verbatim_evidence_search" and candidate.get("is_verbatim_evidence_window"):
        return True
...
def _is_direct_evidence(candidate: dict[str, Any]) -> bool:
    if candidate.get("is_verbatim_evidence_window") or candidate.get("is_artifact_chunk"):
        return True
...
def _raw_evidence_missing(
    ...
    *,
    ...
    verbatim_evidence_search_count: int,
) -> bool:
    ...
    if raw_mode != "verbatim" and not retrieval_plan.exact_recall_mode and not retrieval_plan.require_evidence_regrounding:
        return False
    return direct_evidence_count == 0 and verbatim_evidence_search_count == 0
