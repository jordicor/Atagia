...
def render_answer_privacy_note(context: PromptAuthorityContext | None) -> str:
    ...
    if context.privacy_restrictions_inactive:
        ...
        return (
            ...
            "requested. This permission is scoped to what the final user message "
            "actually asks for; it does not require revealing concrete details while "
            "answering a boundary, applicability, relevance, or permission question. "
            "Treat retrieved source-time privacy requests, disclosure "
...
def render_strong_authority_block(context: PromptAuthorityContext | None) -> str:
    ...
    if context.authenticated_user_is_atagia_master:
        rules.append(
            ...
            "the supplied system context. This permission is scoped to what the "
            "final user message actually asks for; it does not make retrieved "
            "memory an output template and does not require revealing concrete "
            "details while answering a boundary, applicability, relevance, or "
            "permission question. Do not refuse based on privacy, secret "
